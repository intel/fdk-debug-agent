...
TEST_CASE("I2sCapsTlvWrapper", "[WrapperRead]")
{
    ...
    CHECK_THROWS_AS_MSG(
        i2sCapsTlvWrapper.readFrom(invalidSizeTlv, invalidSize), TlvWrapperInterface::Exception,
        "Invalid binary size (" + std::to_string(invalidSize) + " bytes) for a TLV I2sCaps value");
    CHECK(testValueIsValid == false);

    // Read inconsistent value (controller_count to high)
    const size_t inconstistentValueSize = i2sCapsValueSize(0);
    char inconsistentSizeTlv[inconstistentValueSize]{// version
                                                     0x00, 0x00, 0x00, 0x00,
                                                     // controller_count = 2 instead of 0
                                                     0x02, 0x00, 0x00, 0x00};
    CHECK_THROWS_AS_MSG(i2sCapsTlvWrapper.readFrom(inconsistentSizeTlv, inconstistentValueSize),
                        TlvWrapperInterface::Exception, "struct I2sCapabilities inconsistency");
    CHECK(testValueIsValid == false);

    // Read inconsistent value (controller_count to low)
    const size_t inconstistent2ValueSize = i2sCapsValueSize(1);
    char inconsistentSizeTlv2[inconstistent2ValueSize]{// version
                                                       0x00, 0x00, 0x00, 0x00,
    ...
    char emptyControllerBaseAddr[emptyControllerBaseAddrSize]{// version
                                                              0x01, 0x02, 0x03, 0x04,
                                                              // controller_count = 0 instead of 1
                                                              0x00, 0x00, 0x00, 0x00};
    ...
    char twoElementsControllerBaseAddr[twoElementsControllerBaseAddrSize]{
        ...
        0x0E, 0x0F, 0x09, 0x08};
    CHECK_NOTHROW(i2sCapsTlvWrapper.readFrom(twoElementsControllerBaseAddr,
